...
// Фазы 1/2; данные хранятся полями на этом документе (employees,
// gradeTargets, assignments), как и cycles/{cycleId} хранит employees и
// roleAssignments — тот же паттерн, другая коллекция. Чтение — через живую
// подписку onSnapshot прямо в TalentMapTab.jsx (тот же приём, что
// AdminDashboard.jsx уже использует для cycles/{cycleId}), так что этот
// модуль отвечает только за запись.
